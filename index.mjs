      ...
      try {
        ...
        if (isVendedor) {
          ...
          if (!batchId) {
            return json(422, {
              ok: false,
              message: 'No tenés un lote activo asignado para agregar contactos.'
            });
          }

          const manualPayload = {
            nombre: contactPayload.nombre,
            apellido: contactPayload.apellido,
            telefono: contactPayload.telefono,
            celular: contactPayload.celular,
            correo_electronico: contactPayload.correo_electronico || contactPayload.email || contactPayload.correoElectronico || null,
            documento: contactPayload.documento,
            origen_dato: 'manual',
            gestion_id: gestId,
            seller_id: dbUser.id
          };
  ...
  if (method === "POST" && path.endsWith("/api/recupero/lotes")) {
    try {
      ...
      const candidatoIds = Array.isArray(body?.candidato_ids)
        ? body.candidato_ids.filter(Boolean)
        : Array.isArray(body?.contact_ids)
          ? body.contact_ids.filter(Boolean)
          : [];
      ...
      if (!candidatoIds.length || !sellerIds.length) {
        return json(400, { ok: false, message: "candidato_ids y seller_ids son requeridos" });
      }
      ...
      try {
        ...
        if (updateRes.rows.length) {
          await client.query(
            ...
            WHERE id = ANY($3::uuid[])
            `,
            [sellerId, notas, updateRes.rows.map((r) => r.id)]
          );
        }
        ...
        return json(201, {
          ok: true,
          assigned_count: updateRes.rows.length,
          seller_id: sellerId
        });
      } catch (err) {
        await client.query("ROLLBACK");
      ...
      try {
        ...
        const result = await client.query(
          ...
          WHERE lcs.assigned_to = $1
            ...
            ${tabWhere}
          ORDER BY lcs.intentos ASC, d.created_at DESC, lcs.contact_id ASC
          LIMIT $5 OFFSET $6
          `,
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (message.includes('contact_import_batches_type_check') || message.includes('contact_import_batches_import_type_check')) {
            await client.query(
              `
            ...
            await client.query(
              `
              ALTER TABLE public.contact_import_batches
              DROP CONSTRAINT IF EXISTS contact_import_batches_import_type_check;
              `
            );
            await client.query(
              `
