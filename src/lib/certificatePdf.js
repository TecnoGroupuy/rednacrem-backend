...
export function generateCertificatePdf(payload) {
  ...
  const body1 = [
    "A quien corresponda:",
    `Por medio de la presente se deja constancia que ${[client.nombre, client.apellido].filter(Boolean).join(" ")} poseedor del documento de identidad ${client.documento || "-"} ha contratado el servicio de la Red Nacional de Crematorios con fecha ${saleDateLabel}. La cobertura contara con un plazo de carencia de ${carenciaMeses} meses para la adquisicion de derechos. Una vez transcurrido este periodo, el cliente contara con los derechos del servicio de cremacion funeraria de restos de la Red Nacional de Crematorios. La misma incluye traslado de restos, urna cineraria, servicios notariales y sala de cremacion privada, a coordinarse por REDNACREM dentro de los limites del territorio nacional, siempre y cuando el titular del servicio se encuentre al dia con las cuotas de la afiliacion.`
  ];
